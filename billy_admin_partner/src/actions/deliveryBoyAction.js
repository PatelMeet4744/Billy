...
export const createDeliveryBoy = (deliveryBoyData) => async (dispatch) => {
  try {
    ...
    const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
    axios.defaults.headers.common["Authorization"] = token;
    const { data } = await axios.post('http://localhost:4000/api/deliveryBoy', deliveryBoyData, config);
...
export const deletedeliveryBoy = (id) => async (dispatch) => {
  try {
    ...
    const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
    axios.defaults.headers.common["Authorization"] = token;
    const { data } = await axios.delete(`http://localhost:4000/api/deliveryBoy/${id}`, '', config);
...
export const updateDeliveryBoy = (deliveryBoyId, deliveryBoyData) => async (dispatch) => {
  try {
    ...
    const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
    axios.defaults.headers.common["Authorization"] = token;
    const {data} = await axios.put(`http://localhost:4000/api/deliveryBoy/${deliveryBoyId}`, deliveryBoyData, config);
...
export const getsingledeliveryboy = (id) => async (dispatch) => {
  try {
      ...
      const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
      axios.defaults.headers.common["Authorization"] = token;
      const { data } = await axios.get(`http://localhost:4000/api/deliveryBoy/${id}`, '', config);
